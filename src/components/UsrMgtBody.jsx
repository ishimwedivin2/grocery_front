...
const UserMgtBody = () => {
  ...
  const fetchUsers = async () => {
    const response = await fetch(
      `https://webtechcontentbackend-production.up.railway.app/admin/users?pageNo=${currentPage}&pageSize=${pageSize}`
    );
  ...
  const handleDelete = async (userId) => {
    const response = await fetch(
      `https://webtechcontentbackend-production.up.railway.app/admin/users/delete/${userId}`,
      {
        method: "POST",
      }
    );
